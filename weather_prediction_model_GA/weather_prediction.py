...
def evaluate_and_retrain_scalar(X_df, y_ser, cfg, factor_key):
    ...
    weighting_cfg = cfg.get("sample_weighting", {"enabled": False})
    ...
    def fitness_fn(mask, hyperparams):
        ...
        if weighting_cfg.get("enabled", False):
            weight = weighting_cfg.get("positive_class_weight", 1)
            sample_weights = np.where(y_sub > 0, weight, 1)
            model.fit(X_sub, y_sub, sample_weight=sample_weights)
        else:
    ...
    if weighting_cfg.get("enabled", False):
        weight = weighting_cfg.get("positive_class_weight", 1)
        train_weights = np.where(y_train > 0, weight, 1)
        final_test_model.fit(X_train[selected_features], y_train, sample_weight=train_weights)
    else:
    ...
    if weighting_cfg.get("enabled", False):
        weight = weighting_cfg.get("positive_class_weight", 1)
        full_data_weights = np.where(y_ser > 0, weight, 1)
        final_model.fit(X_df[selected_features], y_ser, sample_weight=full_data_weights)
    else:
...
def forecast_scalar(ds_hist: xr.Dataset, var_name: str, model, selected_features, dates_out, cfg):
    ...
    # --- NEW: Create a mask of only the valid (non-NaN) sea points from the last known data ---
    valid_points_mask_flat = ~np.isnan(history[-1].ravel())
    valid_indices = np.where(valid_points_mask_flat)[0]

    for ts in dates_out:
        hs = np.array(history)
        feats = {}
        
        # --- Create features for the ENTIRE grid first ---
        ...
        X_full = pd.DataFrame({k: feats[k] for k in selected_features if k in feats})
        
        # --- NEW: Filter the DataFrame to predict ONLY for the valid sea points ---
        X_pred_sea_only = X_full.iloc[valid_indices]

        # --- Predict on the filtered data ---
        y_sea_only = model.predict(X_pred_sea_only)

        # --- NEW: Create a full-sized grid of NaNs and place the predictions back ---
        y_flat = np.full(nlat * nlon, np.nan)
        y_flat[valid_indices] = y_sea_only
...
def forecast_wcd(ds_hist: xr.Dataset, speed_name: str, deg_name: str, model, selected_features, dates_out, cfg):
    ...
    # --- NEW: Create a mask of only the valid (non-NaN) sea points ---
    valid_points_mask_flat = ~np.isnan(h_speed[-1].ravel())
    valid_indices = np.where(valid_points_mask_flat)[0]
    ...
    for ts in dates_out:
        ...
        # --- NEW: Filter to predict only for valid sea points ---
        X_pred_sea_only = X_full.iloc[valid_indices]
        
        pred_sea_only = model.predict(X_pred_sea_only)

        # --- NEW: Create full-sized NaN grids and place predictions back ---
        sp_flat = np.full(nlat * nlon, np.nan)
        si_flat = np.full(nlat * nlon, np.nan)
        co_flat = np.full(nlat * nlon, np.nan)
        
        sp_flat[valid_indices] = pred_sea_only[:, 0]
        si_flat[valid_indices] = pred_sea_only[:, 1]
        co_flat[valid_indices] = pred_sea_only[:, 2]

        mag = np.sqrt(si_flat**2 + co_flat**2)
        # Avoid division by zero for NaN points
...
def _save_stack_as_nc(varname, forecasts, cfg, filename):
    ...
    stack_in = np.stack([forecasts[d] for d in dates_sorted])
...
def main(config_path):
    ...
    if factor in ("rain", "waves", "ice"):
        ...
        # This check is currently commented out, but you can add a threshold in your config
        # if model is None:
