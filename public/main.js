...
// This script handles the user interface and makes API calls
// to a separate server for route calculations.
// ============================================================
...
function drawMultiColorPath(path) {
    ...
    let currentSegment = [];
    let currentTerrain = path[0].onLand;
    const waterStyle = { color: '#3b82f6', weight: 3 };
    const landStyle = { color: '#10b981', weight: 4, dashArray: '5, 5' };
...
function calculateAndDisplayMetrics(path, speed) {
...
const CustomControl = L.Control.extend({
    ...
    onAdd: function() {
        ...
        container.innerHTML = `<a href="#" id="${this.options.id}" title="${this.options.title}" role="button" class="custom-control bg-gray-700 hover:bg-gray-600 flex items-center justify-center w-9 h-9 rounded-md shadow-md">${this.options.icon}</a>`;
