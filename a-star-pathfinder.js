...
class AStarPathfinder {
    /**
     * MODIFIED: Finds the most fuel-efficient path, now handling No-Go zones.
    ...
    findPath(landGrid, startLatLng, endLatLng, params, envCache, noGoZones = []) {
        ...
        for (const strategy of strategies) {
            ...
            // MODIFIED: Pass noGoZones to the core algorithm
    ...
    /**
     * MODIFIED: The core A* algorithm now accepts noGoZones.
     */
    runAStar(startNode, endNode, landGrid, params, envCache, strategy, noGoZones) { 
        ...
        while (openSet.size() > 0) {
            ...
            for (const neighbor of neighbors) {
                ...
                // MODIFIED: Pass strategy and noGoZones to cost calculation
                const fuelForSegment = this.calculateSegmentCost(currentNode, neighbor, landGrid, params, envCache, strategy, noGoZones);
    ...
    recalculateTotalFuel(path, landGrid, params, envCache, strategy, noGoZones = []) {
        ...
        for (let i = 1; i < path.length; i++) {
            ...
            const segmentFuel = this.calculateSegmentCost(fromNode, toNode, landGrid, params, envCache, strategy, noGoZones);
    ...
    /**
     * NEW: Checks if a point is inside any of the No-Go Zone polygons.
     * Uses the ray-casting algorithm.
     * @param {object} point - The point to check { lat, lng }.
     * @param {Array<object>} noGoZones - Array of GeoJSON polygons.
     * @returns {boolean} True if the point is inside a zone.
     */
    isPointInNoGoZone(point, noGoZones) {
        if (!noGoZones || noGoZones.length === 0) return false;

        for (const zone of noGoZones) {
            // Assuming the first ring is the outer boundary
    ...
    // MODIFIED: Cost function now accepts and processes noGoZones
    calculateSegmentCost(fromNode, toNode, grid, params, envCache, strategy = 'balanced', noGoZones = []) {
        ...
        // NEW: Apply a heavy penalty for entering a No-Go Zone
        const toNodeLatLng = grid.gridToLatLng(toNode.x, toNode.y);
        if (this.isPointInNoGoZone(toNodeLatLng, noGoZones)) {
            costMultiplier *= 1000; // Large penalty to strongly discourage entering
        }
    ...
    calculateFuelPerKm(params) {
        // Assuming k, F, S are provided, if not, default to avoid errors
        const { speed, hpReq, fuelRate, k = 0.005, baseWeight, load, F = 1, S = 1 } = params;
        const speedKmh = speed * 1.852;
    
        if (speedKmh <= 0) return Infinity;
        // Simplified formula, ensure all params are numbers
        ...
        return fuelPerKm || 0.1; // Return a small default if calculation fails
    }
